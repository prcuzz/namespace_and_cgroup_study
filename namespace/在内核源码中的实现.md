...
# User namespaces
 ...
 User namespace 是最核心最复杂的，因为 user ns 是用来隔离和分割管理权限的，管理权限实质分为两部分 `uid/gid` 和 `capability`。 
...
- 进程本身还有一个 user_namespace，为 task->real_cred->user_ns，调用 getpid()/getuid() 所获得的返回值就是由这个 user_namespace 决定的。
